...
json UserRepository::get_status_file(const Subject &subject,
    ...
    std::string query =
            ...
            "chunks where c_username = '?' AND(device_id != "+std::to_string(subject.get_device_id())+") AND c_lastmod > "+std::to_string(last_check)+" "
            ...
            "last_page from chunks where c_username = '?' AND(device_id != "+std::to_string(subject.get_device_id())+") AND "
