...
size_t DBRepository::insertUsernameInDB(const std::string &username) {
  ...
  std::string db_selected_escaped =
      mysqlConn->escapeString(std::to_string(db_selected));

  query = "INSERT INTO UsersDB(username,db) VALUES('" + username_escaped +
          "'," + db_selected_escaped + ");";
  ...
  query =
      "UPDATE db_status SET usc= usc + 1 WHERE db=" + db_selected_escaped + "";
  stmt = std::unique_ptr<sql::Statement>{
      std::move(mysqlConn->createStatement())}; // ricordare al posto di 0, di
                                                // mettere il vero valore
  stmt->execute(query);
