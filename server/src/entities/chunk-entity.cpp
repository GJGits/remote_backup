...
ChunkEntity::ChunkEntity(const DeleteChunkDTO &del_chunk):username{std::move(del_chunk.getusername())},
                                             id_chunk{del_chunk.getchunk_id()},
                                             path_file{std::move(del_chunk.getfile_path())},
                                             last_mod{del_chunk.gettimestamp_locale()}{};


ChunkEntity::ChunkEntity(const GetChunkDTO &get_chunk):username{std::move(get_chunk.getusername())},
                                                          id_chunk{get_chunk.getchunk_id()},
...
std::shared_ptr<std::vector<char>> ChunkEntity::getchunk_body() const { return chunk_body; }