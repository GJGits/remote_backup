...
std::shared_ptr<std::vector<char>> PutChunkDTO::getchunk_body() const { return chunk_body; }
int PutChunkDTO::gettimestamp_locale() const { return timestamp_locale; }



const void PutChunkDTO::fill(const http::server::request &req) { //todo: passarlo per riferimento, non per copia
