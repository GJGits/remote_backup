...
TestController::handle(const http::server::request &req) {
  if (req.method == "GET") {
    ...
    if (std::regex_search(req.uri.begin(), req.uri.end(), match,
      ...
      return MakeReply::make_1line_jsonReply<json>("list", list, http::server::reply::ok);
...
void TestController::delete_them_all() {
    std::shared_ptr<TestService> test_service = TestService::getInstance();
    test_service->cleanDB(); }

const json TestController::get_test_database_table_name(const GetTestDatabaseDTO &get_test_database) {
    std::shared_ptr<TestService> test_service = TestService::getInstance();
    return test_service->getTestDatabaseTable(get_test_database);
}

const json TestController::get_test_filesystem_filename(const GetTestFilesystemFilenameDTO &get_test_filesystem_filename_dto) {
    std::shared_ptr<TestService> test_service = TestService::getInstance();
