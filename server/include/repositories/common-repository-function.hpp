...
public:
  static std::unique_ptr<sql::Statement>
  common_preparation_query(std::string &query,
    ...
    return std::unique_ptr<sql::Statement>{
        std::move(mysqlConn->createStatement())};
  }
  ...
  static bool update_query(std::string &query,
    ...
    common_preparation_query(query, entries_of_query, db_selected)
        ->executeQuery(query);
    return true;
  }
  ...
  select_query(std::string &query, std::list<std::string> &entries_of_query,
    ...
    return std::unique_ptr<sql::ResultSet>{
        std::move(common_preparation_query(query, entries_of_query, db_selected)
                      ->executeQuery(query))};
  }
};