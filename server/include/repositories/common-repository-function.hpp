...
public:
  ...
  static bool update_query(std::string &query,
    ...
    common_preparation_query(query, entries_of_query, db_selected)->execute(query);
    return true;
  }
  ...
  select_query(std::string &query, std::list<std::string> &entries_of_query,
               size_t db_selected) {
      return  std::unique_ptr<sql::ResultSet>{std::move(common_preparation_query(query, entries_of_query, db_selected)->executeQuery(query))};
  }
};